...
-- Standardize last_tracking_at to timestamptz (Requirement 6/3)
DO $$ 
BEGIN
    IF EXISTS (
        ...
        WHERE table_schema = 'public' 
          ...
          AND data_type IN ('text', 'bigint', 'character varying')
    ) THEN
        ALTER TABLE public.walk_sessions 
        ALTER COLUMN last_tracking_at TYPE timestamp with time zone 
        USING (
            CASE 
                WHEN data_type = 'bigint' THEN to_timestamp(last_tracking_at::bigint)
                WHEN data_type IN ('text', 'character varying') AND last_tracking_at ~ '^[0-9.]+$' THEN to_timestamp(last_tracking_at::double precision)
                ELSE NULL 
            END
        );
    END IF;
...
-- REVOKE PUBLIC ACCESS (Requirement 3)
...
DECLARE
    ...
    _session record;
BEGIN
    -- 1. Identity & Role check (Requirement 5.A)
    ...
    -- Monotonicity check (Requirement 5.B)
    ...
    IF _profile.current_walk_id IS NOT NULL THEN
        -- Verify walker owns the current session (Requirement 4 & 5.E)
        ...
        IF FOUND AND _session.walker_id = _walker_id AND _session.current_status IN ('accepted', 'heading_to_pickup', 'arrived', 'in_progress', 'returning') THEN
            ...
            -- TRAIL APPEND (Requirement 5.F/G/H/I handled internally)
