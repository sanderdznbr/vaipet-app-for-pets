...
-- 1. Renomear pin_hash para pin_code e adicionar e2e_run_id
DO $$ 
BEGIN
...
ALTER TABLE public.walk_pickup_codes ADD COLUMN IF NOT EXISTS e2e_run_id text;
...
-- 3. Limpar funções antigas para evitar conflito de parâmetros
...
-- 4. Função customer_get_pickup_code serializada com FOR UPDATE
...
BEGIN
    ...
    INSERT INTO public.walk_pickup_codes (
        ...
        0,
        (SELECT e2e_run_id FROM public.walk_sessions WHERE id = walk_id)
    )
...
-- 5. Função petwalker_confirm_pickup robusta
...
-- 6. Revoke/Grant explicit em todas as RPCs operacionais com assinaturas corretas
REVOKE ALL ON FUNCTION public.petwalker_arrive_pickup(uuid, float8, float8, float8) FROM PUBLIC, anon;
