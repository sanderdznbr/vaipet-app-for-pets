-- 1. Unificar nomes de colunas: usar pin_hash em walk_pickup_codes
-- (Garantir que a tabela tenha pin_hash)
...
-- 2. Recriar RPCs com consistência total
...
BEGIN
    ...
    IF _pin IS NULL THEN
        ...
        INSERT INTO public.walk_pickup_codes (session_id, pin_hash, expires_at, attempts)
        VALUES (_session_id, _pin, now() + interval '30 minutes', 0)
...
BEGIN
    ...
    SELECT * INTO _code_record FROM public.walk_pickup_codes WHERE session_id = _session_id FOR UPDATE;
    ...
    SET current_status = 'in_progress', status = 'in_progress', pickup_confirmed_at = now(), start_time = now(), updated_at = now()
