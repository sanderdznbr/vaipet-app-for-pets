-- 1. CLEANUP PREVIOUS ATTEMPTS (DROPPING OBSOLETE SIGNATURES)
...
DECLARE
    ...
    v_offer_id uuid;
BEGIN
    -- Check if user is authenticated and is a walker
    IF v_walker_id IS NULL THEN RAISE EXCEPTION 'Não autenticado'; END IF;
    
    -- Lock the session for matching logic
    ...
    -- Verify walker profile and availability
    IF NOT EXISTS (
    ...
    -- Verify the specific offer exists
    ...
    IF v_offer_id IS NULL THEN
        RAISE EXCEPTION 'Oferta não encontrada ou já expirada.';
    END IF;
...
-- 4. UPDATE WALKER LOCATION (Canonical SRID 4326)
...
BEGIN
    ...
    -- Update last known location (using SRID 4326 geography)
    UPDATE public.petwalker_profiles
    ...
    -- Log tracking if in active walk
...
BEGIN
    ...
    INTO v_initial_radius, v_max_radius, v_increment, v_expiry_min, v_exp_interval_min
    ...
    LOOP
        IF r_session.search_started_at IS NULL THEN
            ...
            SET search_started_at = now(),
                matching_expires_at = now() + (v_expiry_min * interval '1 minute')
            WHERE id = r_session.id;
            r_session.search_started_at := now();
        END IF;

        v_elapsed_min := EXTRACT(EPOCH FROM (now() - r_session.search_started_at)) / 60;
        v_radius := v_initial_radius + (FLOOR(v_elapsed_min / NULLIF(v_exp_interval_min, 0)) * v_increment);
        IF v_radius > v_max_radius THEN v_radius := v_max_radius; END IF;

        FOR r_walker IN
            SELECT user_id, ST_Distance(last_known_location, r_session.meeting_point_geom) as dist
...
-- 7. SECURE SESSION WALKER PROFILE
