...
-- Migration aditiva para endurecer segurança da RPC get_available_walk_offers
-- Timestamp superior a 20260814182237

-- 1. Remove a função anterior para evitar conflitos
DROP FUNCTION IF EXISTS public.get_available_walk_offers();

-- 2. Recria com validações de elegibilidade baseadas em roles e segurança Zero-Trust
...
BEGIN
  ...
  -- 3. Exigir explicitamente a role 'petwalker' via user_roles
  SELECT EXISTS (
  ...
  IF NOT _has_role THEN
    RETURN; -- Não possui a role necessária
  END IF;

  -- 4. Validar se o usuário é um PetWalker aprovado e disponível
  SELECT * INTO _walker_profile
  FROM public.petwalker_profiles
  WHERE user_id = _authenticated_user_id;

  -- Validações de elegibilidade estritas com segurança contra NULL
  ...
  -- 5. Retornar ofertas específicas para este Walker
  RETURN QUERY
  SELECT 
    o.id,
    o.session_id,
...
-- 6. Revogações e permissões explícitas (Removido service_role pois auth.uid() é exigido)
