-- PHASE 4.3 — PATCH 1B — FINAL AUDIT HARDENING

-- 1. HARDEN SYNC LOGIC (Fail-closed on divergence)
CREATE OR REPLACE FUNCTION public.fn_sync_walk_status_v3()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        ...
        ELSIF NEW.status IS NOT DISTINCT FROM NEW.current_status THEN
            NULL; -- Accept
        -- Case D: Divergence
        ELSE
    ...
    ELSIF TG_OP = 'INSERT' THEN
        IF NEW.current_status IS NOT NULL AND NEW.status IS NOT NULL THEN
            IF NEW.current_status IS DISTINCT FROM NEW.status THEN
...
CREATE TRIGGER tr_01_sync_walk_status
    ...
    FOR EACH ROW
    EXECUTE FUNCTION public.fn_sync_walk_status_v3();
