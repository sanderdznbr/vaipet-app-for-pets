...
BEGIN
    ...
    IF _last_tracking_at IS NOT NULL AND (EXTRACT(EPOCH FROM NOW())::bigint - _last_tracking_at) < 5 THEN
        RETURN;
    END IF;
    ...
    SET 
        ...
        last_tracking_at = EXTRACT(EPOCH FROM NOW())::bigint,
        updated_at = NOW()
    WHERE id = _session_id;
END;
$$;
