...
-- 1. Redefinir funções operacionais com lógica hardened
...
BEGIN
    ...
    SELECT * INTO _session_record FROM public.walk_sessions WHERE id = _session_id;
    ...
    IF _pin IS NULL THEN
        ...
        ON CONFLICT (session_id) DO UPDATE SET pin_hash = EXCLUDED.pin_hash, attempts = 0, expires_at = (now() + interval '30 minutes'), created_at = now();
    END IF;
    RETURN _pin;
END;
$$;
...
BEGIN
    IF _user_id IS NULL THEN RAISE EXCEPTION 'Não autenticado.'; END IF;
    IF NOT public.has_role(_user_id, 'petwalker') THEN RAISE EXCEPTION 'Apenas PetWalkers.'; END IF;
    UPDATE public.walk_sessions SET current_status = 'heading_to_pickup', updated_at = now()
    WHERE id = _session_id AND walker_id = _user_id AND current_status = 'accepted';
    RETURN FOUND;
END;
$$;
...
BEGIN
    IF _user_id IS NULL THEN RAISE EXCEPTION 'Não autenticado.'; END IF;
    IF _lat IS NULL OR _lng IS NULL OR _lat = 0 OR _lng = 0 THEN RAISE EXCEPTION 'GPS inválido.'; END IF;
    SELECT * INTO _session_record FROM public.walk_sessions WHERE id = _session_id;
    IF NOT FOUND THEN RAISE EXCEPTION 'Sessão não encontrada.'; END IF;
    ...
    IF _session_record.current_status IS DISTINCT FROM 'heading_to_pickup' THEN
        RAISE EXCEPTION 'Status inválido: %', _session_record.current_status;
    END IF;
    IF _session_record.home_location IS NULL OR (_session_record.home_location->>'lng') IS NULL OR (_session_record.home_location->>'lat') IS NULL THEN
        RAISE EXCEPTION 'Localização não definida.';
    END IF;
    _dist_meters := ST_Distance(ST_SetSRID(ST_MakePoint(_lng, _lat), 4326)::geography, ST_SetSRID(ST_MakePoint((_session_record.home_location->>'lng')::double precision, (_session_record.home_location->>'lat')::double precision), 4326)::geography);
    IF _dist_meters > (150 + LEAST(_accuracy, 50)) THEN
        RAISE EXCEPTION 'Muito longe (dist: %m).', round(_dist_meters::numeric, 2);
    END IF;
    UPDATE public.walk_sessions SET current_status = 'arrived', updated_at = now() WHERE id = _session_id;
    RETURN TRUE;
END;
$$;
...
BEGIN
    IF _user_id IS NULL THEN RAISE EXCEPTION 'Não autenticado.'; END IF;
    IF _pickup_code IS NULL OR _pickup_code !~ '^[0-9]{6}$' THEN RAISE EXCEPTION 'PIN inválido.'; END IF;
    PERFORM 1 FROM public.walk_sessions WHERE id = _session_id FOR UPDATE;
    SELECT * INTO _code_record FROM public.walk_pickup_codes WHERE session_id = _session_id;
