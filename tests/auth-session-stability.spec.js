...
import { expectNeverBlankAfterFirstPaint, sampleRender, seedSession, } from "./utils/render-probe";
/**
            ...
            new MutationObserver((records) => {
                for (const r of records) {
                    ...
                    for (const removed of Array.from(r.removedNodes)) {
                        if (removed.nodeType === 1 && root.childElementCount === 0)
                            w.__rootTeardowns++;
                    }
                    for (const added of Array.from(r.addedNodes)) {
                        if (added.nodeType === 1 &&
                            added.querySelector?.('[data-testid="splash-screen"]')) {
                            w.__splashMounts++;
                        }
                    }
                }
...
const readCounters = (page) => page.evaluate((key) => ({
    docInits: Number(sessionStorage.getItem(key) || "0"),
    rootTeardowns: window.__rootTeardowns ?? 0,
    splashMounts: window.__splashMounts ?? 0,
}), DOC_INITS_KEY);
...
async function returnHome(page) {
    if (new URL(page.url()).pathname === "/inicio")
        return;
    if (!(await navigateTo(page, "Home")))
        await page.goBack();
...
test.describe("Sessão autenticada — refresh e troca de rota sem remount/flash", () => {
    ...
    test("refreshing while authenticated keeps the route and the session", async ({ page }) => {
        ...
        await bootHome(page);
        const before = await page.evaluate((key) => (key ? window.localStorage.getItem(key) !== null : null), process.env.LOVABLE_BROWSER_SUPABASE_STORAGE_KEY ?? "");
        for (let i = 1; i <= 3; i++) {
            ...
            if (authenticated) {
                const stillSignedIn = await page.evaluate((key) => window.localStorage.getItem(key) !== null, process.env.LOVABLE_BROWSER_SUPABASE_STORAGE_KEY);
    ...
    test("route changes are client side — no document reload, no root remount", async ({ page }) => {
        ...
        for (const item of NAV) {
            if (!(await navigateTo(page, item.label)))
                continue;
            await page.waitForTimeout(1_200);
            visited.push(new URL(page.url()).pathname);
            const now = await readCounters(page);
            expect(now.docInits, `Navigating to ${item.path} triggered a full document reload (SPA routing broken)`).toBe(start.docInits);
            expect(now.rootTeardowns, `Navigating to ${item.path} unmounted the whole React root (remount flash)`).toBe(0);
            expect(now.splashMounts, `Navigating to ${item.path} replayed the splash screen`).toBe(0);
