...
const log = (msg) => console.log(`[${new Date().toISOString()}] [walker-acceptance] ${msg}`);
let admin;
test.beforeAll(async () => {
    admin = createClient(SUPABASE_URL, SERVICE_KEY, { auth: { persistSession: false } });
});
async function provisionWalker(runId) {
...
async function createOwnerAndRequest(runId) {
...
test("walker-acceptance: ACEITE ISOLADO", async ({ browser }) => {
    ...
    try {
        ...
        await expect.poll(async () => {
            ...
            if (!ok)
                log(`Status atual: ${data?.current_status}, Walker: ${data?.walker_id}`);
            return ok;
