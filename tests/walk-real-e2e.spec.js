...
const log = (msg) => console.log(`[${new Date().toISOString()}] [e2e] ${msg}`);
let admin;
...
async function preflightCleanup() {
    ...
    while (true) {
        ...
        if (users.length === 0)
            break;
        const targets = users.filter(u => u.email?.endsWith("@e2e.vaipet.invalid") &&
...
async function quickCleanup(ids, runId) {
    await failClosedCleanup(admin, ids, runId);
}
async function provisionUser(runId, kind) {
...
async function createAuthedContext(browser, credentials, coords) {
