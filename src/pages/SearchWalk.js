...
import { ArrowLeft, Plus, Minus, Navigation, Sun, Moon, MapPin, Clock, PawPrint, X, Sparkles, Map as MapIcon, Search, Trash2, Loader2, GripVertical, Cloud, CloudRain, CloudSnow, CloudFog, CloudLightning, CloudSun } from 'lucide-react';
...
import { generateRandomWalker, buildBetaWalker, pickTransportForDistance } from '@/lib/walkerProfile';
...
const isRainCode = (code) => [51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82, 95, 96, 99].includes(code);
const classifyRain = (code, precipMm) => {
    ...
    let level;
    if (heavyCode || precipMm >= 4)
        level = 'heavy';
    else if (drizzle || precipMm < 0.6)
        level = 'light';
    else
        level = 'moderate';
    if (level === 'light') {
...
const SearchWalk = () => {
    ...
    const currentRouteCoords = useRef(null);
    ...
    const fetchQuote = useCallback(async (duration, mode) => {
    ...
    const lastMarkerPosRef = useRef(null);
    const pendingLocRef = useRef(null);
    const watchDebounceTimerRef = useRef(null);
    ...
    const [walkType, setWalkType] = useState('livre');
    const [walkTypeNotes, setWalkTypeNotes] = useState('');
    ...
    const [plannedRouteInfo, setPlannedRouteInfo] = useState(null);
    const stopMarkersRef = useRef([]);
    const [currentSessionId, setCurrentSessionId] = useState(null);
    const [sheetExpanded, setSheetExpanded] = useState(true);
    const [walker, setWalker] = useState(() => generateRandomWalker());
    ...
    const [isResuming, setIsResuming] = useState(() => !!searchParams.get('resume'));
    ...
    const commitUserLocation = useCallback((loc, force = false) => {
    ...
    useEffect(() => {
        ...
        if (!sessionId || !user || resumeHandledRef.current)
            return;
        ...
        (async () => {
            try {
                ...
                setWalkType(session.walk_type || 'livre');
                setSelectedMinutes(session.planned_duration_minutes || 30);
                const home = session.home_location;
                if (home && Number.isFinite(home.lng) && Number.isFinite(home.lat)) {
                    ...
                    const fallback = userLocation ?? [-46.6333, -23.5505];
    ...
    useEffect(() => {
        ...
        const smoothLocation = (raw, accuracy) => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition((position) => commitUserLocation([position.coords.longitude, position.coords.latitude], true), () => commitUserLocation([-46.6333, -23.5505], true));
            watchId = navigator.geolocation.watchPosition((position) => {
                const raw = [position.coords.longitude, position.coords.latitude];
        ...
        (async () => {
            try {
                ...
                if (typeof c?.temperature_2m === 'number') {
                    const precip = Math.max(Number(c.precipitation ?? 0) || 0, Number(c.rain ?? 0) || 0, Number(c.showers ?? 0) || 0);
                    setWeather({
    ...
    useEffect(() => {
        if (!mapContainer.current || !userLocation || map.current)
            return;
        ...
        const getUserInitials = (name) => name.split(' ').map(w => w.charAt(0)).join('').toUpperCase().slice(0, 2);
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => {
            try {
                ...
                if (error)
                    throw error;
                const feats = (data?.results || []).map((r) => ({
                    id: r.id,
    ...
    useEffect(() => {
        ...
        const m = map.current;
        const ensureRemoved = () => {
            if (m.getLayer('planned-route'))
                m.removeLayer('planned-route');
            if (m.getSource('planned-route'))
                m.removeSource('planned-route');
            if (m.getLayer('walker-route'))
                m.removeLayer('walker-route');
            if (m.getSource('walker-route'))
                m.removeSource('walker-route');
        ...
        const loopPoints = [userLocation, ...localStops.map((s) => [s.lng, s.lat]), userLocation];
        ...
        const walkerPoints = [userLocation, ...localStops.map((s) => [s.lng, s.lat])];
        ...
        (async () => {
            try {
                ...
                if (m.getSource('planned-route')) {
                    m.getSource('planned-route').setData(data);
                }
                else {
                ...
                if (walkerRes) {
                    ...
                    if (wroute) {
                        const wdata = { type: 'Feature', properties: {}, geometry: wroute.geometry };
                        if (m.getSource('walker-route')) {
                            m.getSource('walker-route').setData(wdata);
                        }
                        else {
    ...
    useEffect(() => {
        ...
        const h = window.innerHeight;
        const safeBottom = parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sai-b') || '0', 10) || 0;
    ...
    const animateRouteUberStyle = (coordinates) => {
        if (!map.current)
            return;
        ...
        const animate = () => {
            ...
            if (target > cur) {
                ...
                const data = { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: slice } };
                if (map.current?.getSource('route'))
    ...
    const addRouteToMap = async (u, w) => {
        ...
        try {
            ...
            if (json.routes?.[0]) {
                ...
                const snappedWalker = coords[coords.length - 1];
                if (walkerMarker.current)
                    walkerMarker.current.setLngLat(snappedWalker);
                setWalkerLocation(snappedWalker);
                setPickupRoute([...coords].reverse());
                setIsDrawingRoute(false);
                const bounds = new mapboxgl.LngLatBounds();
                bounds.extend(u);
                bounds.extend(snappedWalker);
    ...
    const handleSearch = async () => {
        if (!user || selectedPets.length === 0 || !userLocation)
            return;
        try {
            ...
            if (walkerProfile) {
                ...
                if (loc?.coordinates) {
                    ...
                    if (map.current) {
                        ...
                        el.innerHTML = `<div class="absolute inset-0 rounded-full bg-[#31D880] animate-pulse opacity-30"></div><div class="relative w-10 h-10 rounded-full border-2 border-[#31D880] overflow-hidden bg-white shadow-lg"><img src="${walkerProfile.profiles?.avatar_url || '/vaipet-logo.svg'}" alt="Walker" class="w-full h-full object-cover" /></div>`;
...
return (<div className={`fixed inset-0 ${fullscreen ? '' : 'max-w-md mx-auto'}`} style={{ background: isDayMode ? '#F7F5EF' : '#0B1410' }}>
    ...
    map.current.flyTo({ center: userLocation, zoom: 15 }); }} className="w-11 h-11 rounded-full shadow-lg flex items-center justify-center" style={{ background: ui.chip, boxShadow: ui.shadow }}>
      ...
      {showBottomSheet && (<div className="absolute left-3 right-3 z-30" style={{ bottom: 'calc(0.75rem + env(safe-area-inset-bottom))' }}>
          <div className="rounded-[32px] overflow-hidden" style={{
            ...
            <div className="px-5 pt-4 pb-5">
              ...
              {/* STEP 1 — Quando? */}
              {step === 1 && (<div key="s1" className="animate-fade-in">
                ...
                transitionDelay: scheduleMode === 'later' ? '160ms' : '0ms',
                  ...
                  <div className="mb-4 mt-1">
                    ...
                    <div className="flex items-start gap-4 overflow-x-auto pb-1 px-1 -mx-1 scrollbar-none">
                      {pets.length === 0 ? (<button onClick={() => navigate('/add-pet')} className="flex items-center gap-3 w-full p-4 rounded-2xl animate-pulse" style={{ background: ui.inner, border: `1px dashed #31d88044` }}>
                ...
                return (<button key={p.id} onClick={() => {
                        ...
                        opacity: active || selectedPets.length === 0 ? 1 : (selectedPets.length > 0 ? 0.4 : 0.55),
                              ...
                              <span className="text-[11px] font-semibold max-w-[60px] truncate" style={{ color: active ? ui.text : ui.textSoft }}>
                                {p.name}
                              </span>
                            </button>);
            }))}
            ...
            setStep(2); }} disabled={selectedPets.length === 0} className="w-full py-3.5 rounded-full text-white font-extrabold text-sm transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: '#31d880', boxShadow: selectedPets.length > 0 ? '0 10px 24px rgba(49,216,128,0.35)' : 'none' }}>
              ...
              {step === 3 && (<div key="s2" className="animate-fade-in">
                  <p className="text-[11px] font-bold uppercase tracking-wider mb-3 px-1" style={{ color: ui.muted }}>Duração</p>
                  <div className="flex items-center justify-between mb-4 rounded-3xl px-3 py-3" style={{ background: ui.inner, border: ui.borderSoft, boxShadow: '0 2px 6px rgba(0,0,0,0.04)' }}>
                    <button onClick={() => setSelectedMinutes((m) => Math.max(15, m - 15))} disabled={selectedMinutes <= 15} className="w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-90 disabled:opacity-40" style={{ background: '#31d880', boxShadow: '0 6px 16px rgba(49,216,128,0.35)' }} aria-label="Diminuir duração">
                    ...
                    <button onClick={() => setSelectedMinutes((m) => m + 15)} className="w-12 h-12 rounded-full flex items-center justify-center transition-all active:scale-90" style={{ background: '#31d880', boxShadow: '0 6px 16px rgba(49,216,128,0.35)' }} aria-label="Aumentar duração">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setStep(2)} className="px-5 py-3.5 rounded-full text-sm font-bold active:scale-[0.98] transition-all" style={{ background: ui.inner, border: ui.borderSoft, color: ui.text }}>
                      Voltar
                    </button>
                    <button onClick={() => setStep(4)} className="flex-1 py-3.5 rounded-full text-white font-extrabold text-sm transition-all active:scale-[0.98]" style={{ background: '#31d880', boxShadow: '0 10px 24px rgba(49,216,128,0.35)' }}>
                      Continuar
                    </button>
                  </div>
                </div>)}
            ...
            return (<div key="s3" className="animate-fade-in">
                  ...
                  <div className="grid transition-[grid-template-rows,opacity,margin] duration-500" style={{
                    ...
                    <div className="overflow-hidden">
                      ...
                      {localStops.length > 0 && (<div className="flex flex-col gap-2 mb-2">
                          {localStops.map((s, i) => (<div key={s.id} draggable onDragStart={(e) => {
                            ...
                            boxShadow: dragOverIndex === i ? '0 6px 18px rgba(0,0,0,0.15)' : 'none',
                              ...
                              <div className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-extrabold shrink-0 text-white" style={{ background: '#31d880' }}>
                                {i + 1}
                              </div>
                              ...
                              <button onClick={() => setLocalStops((prev) => prev.filter((x) => x.id !== s.id))} className="w-7 h-7 rounded-full flex items-center justify-center active:scale-95" style={{ background: ui.divider, color: ui.text }} aria-label="Remover">
                      ...
                      <div className="flex items-center gap-2 px-3 py-2.5 rounded-full" style={{ background: ui.inner, border: ui.borderSoft }}>
                        {addrLoading ? (<Loader2 className="w-4 h-4 shrink-0 animate-spin" style={{ color: ui.muted }}/>) : (<Search className="w-4 h-4 shrink-0" style={{ color: ui.muted }}/>)}
                        <input value={addrQuery} onChange={(e) => setAddrQuery(e.target.value)} placeholder="Buscar endereço ou local..." className="flex-1 bg-transparent outline-none text-sm font-semibold" style={{ color: ui.text }}/>
                        {addrQuery && (<button onClick={() => { setAddrQuery(''); setAddrSuggestions([]); }} className="shrink-0">
                      ...
                      {plannedRouteInfo && localStops.length > 0 && (<p className="mt-2 text-[11px] font-bold uppercase tracking-wider px-1" style={{ color: ui.muted }}>
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setStep(1)} className="px-5 py-3.5 rounded-full text-sm font-bold active:scale-[0.98] transition-all" style={{ background: ui.inner, border: ui.borderSoft, color: ui.text }}>
                      Voltar
                    </button>
                    <button onClick={() => setStep(3)} disabled={walkType === 'local' && localStops.length === 0} className="flex-1 py-3.5 rounded-full text-white font-extrabold text-sm transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed" style={{ background: '#31d880', boxShadow: '0 10px 24px rgba(49,216,128,0.35)' }}>
                      Continuar
                    </button>
                  </div>
                </div>);
              ...
              {step === 4 && (<div key="s4" className="animate-fade-in">
                  <div className="flex items-center justify-between mb-3 px-1">
                    ...
                    <button onClick={() => setStep(3)} className="text-[11px] font-bold uppercase tracking-wider active:scale-95 transition-transform" style={{ color: ui.muted }}>
                      Voltar
                    </button>
                  </div>
                ...
                : selectedMinutes >= 60
                    ...
                    {walkType === 'local' && localStops.length > 0 && (<div className="px-4 py-3" style={{ borderTop: `1px solid ${ui.divider}` }}>
                        ...
                        <div className="flex flex-col gap-1.5">
                          {localStops.map((s, i) => (<div key={s.id} className="flex items-start gap-2">
                              <div className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-extrabold shrink-0 text-white" style={{ background: '#31d880' }}>{i + 1}</div>
                    ...
                    <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: `1px solid ${ui.divider}` }}>
                      ...
                      <span className="text-base font-extrabold text-[#31d880]">
                        {quoteLoading ? (<Loader2 className="w-4 h-4 animate-spin"/>) : quote ? (Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(quote.total_price_cents / 100)) : (`R$ ${selectedMinutes},00`)}
                      </span>
                    </div>
                  </div>
      ...
      {isSearching && (<div className="absolute left-1/2 z-30 animate-pill-in" style={{ bottom: 'calc(env(safe-area-inset-bottom) + 72px)', transform: 'translateX(-50%)' }}>
          <div className="flex items-center gap-3.5 backdrop-blur-md rounded-full pl-2.5 pr-6 py-2.5 shadow-2xl transition-[width,padding] duration-500 ease-out min-w-[260px]" style={{ background: ui.pillBg, border: ui.border }}>
            <div key={searchStatus} className="flex items-center gap-3.5 animate-pill-content-in">
              {searchStatus === 'found' ? (<div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[#31d880]">
                  <img src={walker.avatar} alt={walker.firstName} className="w-full h-full object-cover"/>
                </div>) : (<div className="relative w-12 h-12 flex items-center justify-center">
            ...
            <button onClick={handleCancel} className="ml-auto w-10 h-10 rounded-full flex items-center justify-center active:scale-90 transition-all shrink-0" style={{ background: isDayMode ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.08)' }} aria-label="Cancelar">
      ...
      {/* Review */}
      {searchStatus === 'reviewing' && (<ReviewWalk onBack={() => setSearchStatus('walking')} onComplete={handleReviewComplete} petName={selectedPets.length === 1 ? selectedPets[0].name : `${selectedPets.length} pets`} walkerName={walker.firstName} walkDuration={walkDuration} isDarkMode={!isDayMode} sessionId={currentSessionId || undefined}/>)}
